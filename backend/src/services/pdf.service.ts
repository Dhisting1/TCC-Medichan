...
Layout espelha o Modelo da Prescrição (Figura 4.7 do TCC).
*/
export function generatePrescriptionPdf(opts: PrescriptionPdfOptions): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor("white")
       .font("Helvetica")
       .fontSize(10)
       .text("Prescrição Médica Digital", W - M - 120, 30, { width: 120, align: "right" });
    ...
    // ── linha divisória ────────────────────────────────────────────────────
    const lineY = 124;
    doc.moveTo(M, lineY).lineTo(W - M, lineY).strokeColor("#E5E7EB").lineWidth(1).stroke();

    // ── bloco "Dados do médico" ────────────────────────────────────────────
    doc.rect(M, 136, W - M * 2, 48).fillColor("#F0F2F0").fill();
    ...
    doc.rect(M, 196, W - M * 2, 64).fillColor("#F9FAFB").fill();
    ...
    doc.moveTo(M, 296).lineTo(W - M, 296).strokeColor("#E5E7EB").lineWidth(1).stroke();
    ...
    // ── rodapé com QR Code ─────────────────────────────────────────────────
    const footerY = 680;
    doc.rect(M, footerY, W - M * 2, 80).fillColor(TEAL).fill();

    doc.fillColor("white").font("Helvetica").fontSize(8)
       .text(
         "RECEITA DIGITAL. SUA AUTENTICIDADE E DISPENSAÇÃO DEVEM SER VALIDADAS PELO\n" +
         "QR CODE CONFORME INSTRUÇÕES ABAIXO.",
         M + 12, footerY + 12, { width: W - M * 2 - 160 }
       );

    doc.fillColor(LIME).font("Helvetica").fontSize(8)
       .text(`ID: ${opts.prescriptionId}`, M + 12, footerY + 42, { width: W - M * 2 - 160 });
    ...
    doc.fillColor("#9CA3AF").font("Helvetica").fontSize(8)
       .text(
         ...
         M, 772, { align: "center", width: W - M * 2 }
       );
