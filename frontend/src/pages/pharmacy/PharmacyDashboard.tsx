...
`;
const SideLbl = styled.p`
  font-size: 11px;
  color: #9ca3af;
  margin-bottom: 2px;
`;
const SideVal = styled.p`
  font-size: 13px;
  color: #374151;
  font-weight: 500;
`;
...
const PrescCard = styled.div<{ selected?: boolean }>`
  ...
  &:hover {
    border-color: ${TEAL};
    box-shadow: 0 2px 8px rgba(11, 53, 48, 0.08);
  }
`;

const PrescCardRow = styled.div`
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 6px;
`;
const PrescPatient = styled.span`
  font-size: 14px;
  font-weight: 600;
  color: ${TEAL};
`;
const PrescMed = styled.span`
  font-size: 13px;
  color: #374151;
`;
const PrescDate = styled.span`
  font-size: 11px;
  color: ${MUTED};
`;
const PrescId = styled.span`
  font-size: 11px;
  color: #9ca3af;
...
export default function PharmacyDashboard() {
  ...
  async function handlePatientSearch() {
    ...
    try {
      const res = await api.get(
        `/users/search?q=${encodeURIComponent(patientQ.trim())}`,
      );
  ...
  async function handleSearch() {
    ...
    try {
      ...
      setFound({
        id: searchId.trim(),
        ...res.data,
        status: res.data.status === "VALID" ? "ACTIVE" : res.data.status,
      });
    } catch {
      setMsg("Receita não encontrada ou inválida.");
      setMsgOk(false);
    } finally {
      setSearching(false);
    }
  ...
  async function handleDispense() {
    ...
    try {
      ...
      setMsg(err.response?.data?.error || "Erro ao dispensar.");
      setMsgOk(false);
    } finally {
      setDispensing(false);
    }
        ...
        <PageGrid>
          <Card>
            <CardHeader>
              ...
              <div>
                <HeaderTitle>Validar nova receita</HeaderTitle>
                <br />
            ...
            {found ? (
              <InfoAreaFilled>
                <InfoRow>
                  <InfoKey>Paciente:</InfoKey>{" "}
                  <InfoVal>{found.patient || "—"}</InfoVal>
                </InfoRow>
                <InfoRow>
                  <InfoKey>Medicamento:</InfoKey>{" "}
                  <InfoVal>{found.medication || "—"}</InfoVal>
                </InfoRow>
                <InfoRow>
                  <InfoKey>Dosagem:</InfoKey>{" "}
                  <InfoVal>{found.dosage || "—"}</InfoVal>
                </InfoRow>
                <InfoRow>
                  <InfoKey>Data:</InfoKey>{" "}
                  <InfoVal>
                    {found.createdAt ? fmtDate(found.createdAt) : "—"}
                  </InfoVal>
                </InfoRow>
                <InfoRow>
              ...
              <InfoArea>
                <PlaceholderText>
                  INFORMAÇÕES CONTIDAS NA RECEITA
                </PlaceholderText>
              </InfoArea>
            )}
            ...
            <SideCard>
              ...
              <SideBody>
                {selected ? (
                  <>
                    <SideField>
                      <SideLbl>Paciente:</SideLbl>{" "}
                      <SideVal>{selected.patient}</SideVal>
                    </SideField>
                    <SideField>
                      <SideLbl>Medicamento:</SideLbl>{" "}
                      <SideVal>{selected.medication}</SideVal>
                    </SideField>
                    <SideField>
                      <SideLbl>Dosagem:</SideLbl>{" "}
                      <SideVal>{selected.dosage}</SideVal>
                    </SideField>
                    <SideField>
                      <SideLbl>Data:</SideLbl>{" "}
                      <SideVal>{fmtDate(selected.createdAt)}</SideVal>
                    </SideField>
          ...
          {!loadingH && dispensed.length > 0 && (
            <Table>
              ...
              <tbody>
                {dispensed.map((p) => (
                  <tr key={p.id}>
                    <Td style={{ fontWeight: 600, color: TEAL }}>
                      {p.patient}
                    </Td>
                    <Td>{p.medication}</Td>
                    <Td>{p.dosage}</Td>
